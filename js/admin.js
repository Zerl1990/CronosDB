...
function createNominationView() {
  ...
  $('#edit-table > thead').append("<tr><th># Nomination ID</th><th># Award ID</th><th>Award Name</th><th># Nom Category ID</th><th>Category Name</th><th># Actor ID</th><th>Actor Name</th><th>Actor Nationality</th><th>Actor Birthday</th><th>Year</th><th>Won</th><th>Movie ID</th><th>Movie Name</th></tr>");

  fillAwardsDropdown();
  fillActorsDropdown();
  fillNomNCMovieDropdown();
...
function fillNomNCMovieDropdown() {
  $('#nom-nc-movie-id').empty();
  $.ajax({
    ...
    success: function(response) {
      ...
      for (var j = 0; j < length; j++) {
        $('#nom-nc-movie-id').append("<option value='" + response[j].id + "'>" + response[j].name + "</option>");
      }
    },
    error: function(response) {
      var jsonResponse = response.responseJSON;
...
function createNomSubView(award) {
  ...
  $.ajax({
    ...
    success: function(response) {
      ...
      var length = response.length;
      for (var i = 0; i < length; i++) {
        $('#edit-table > tbody').append("<tr><td>" + response[i].id + "</td><td>" + response[i].award_id + "</td><td>" + response[i].award_name + "</td><td>" + response[i].nc_id + "</td><td>" + response[i].category_name + "</td><td>" + response[i].actor_id + "</td><td>" + response[i].actor_name +"</td><td>" + response[i].actor_nationality + "</td><td>" + response[i].actor_birth_date + "</td><td>" + response[i].year + "</td><td>" + response[i].won + "</td><td>" + response[i].movie_id + "</td><td>" +response[i].movie + "</td><td><button class='edit-button btn btn-warning btn-sm'>Edit</button></td></tr>");
      }
      $('.edit-button').click(function() {
        resultArray = [];
        $(this).closest("td").siblings().each(function () {
        ...
        $('#nom-nc-movie-id').val(resultArray[11]);
      });
...
function post_nom() {
  ...
  var nomMovie = $('#nom-nc-movie-id').val();
